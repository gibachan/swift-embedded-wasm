...
- Bulk Memory 命令（0xFC プレフィックス）— メモリ操作
  ...
  - `memory.fill`（0xFC 0x0B）: n バイトをバイト値 val で埋める
- Bulk Table 命令（0xFC プレフィックス）— テーブル操作
...
クロスモジュール・リンキングとは、複数の `.wasm` ファイル間でテーブルやメモリを `import`/`export` で共有し、モジュールを動的に結合する仕組みである。主に以下のユースケースで使われる：

- C/C++ の動的リンク（Emscripten の dlopen 相当）
- プラグインシステム
- Wasm Component Model の前身としてのモジュール分割

**本プロジェクトでは実装を見送る。** 理由は以下のとおり：

1. **組み込み環境のユースケースに合致しない**: Raspberry Pi Pico 上で実行する想定のバイナリは単一の `.wasm` ファイルである。複数モジュールを動的にリンクするリソース（RAM・Flash）も想定しない。
