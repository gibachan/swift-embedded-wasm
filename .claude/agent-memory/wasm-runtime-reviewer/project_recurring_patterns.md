...
61. **`pushFrame(argCount:)` hot-path refactor — `argsStart` underflow not guarded**: After changing `pushFrame` to take `argCount: Int` instead of `callArgs: [Value]`, the function computes `let argsStart = valueStack.count - argCount` without first checking that `valueStack.count >= argCount`. The callers (`.call` and `.callIndirect`) do `guard valueStack.count >= argCount else { throw .stackUnderflow }` before calling, so in practice underflow cannot occur, but the guard is absent INSIDE `pushFrame` itself. The host-call branch also uses `let argsStart = valueStack.count - argCount` without a local guard. In a well-typed module this is always safe; but a defensive `guard argsStart >= 0` inside `pushFrame` would make the invariant explicit and protect against future caller bugs.

62. **`pushFrame` `stackBase` timing is correct**: `stackBase = valueStack.count` is set AFTER `valueStack.removeLast(argCount)`, so it points to the stack top after arguments are consumed. This is the correct invariant: `stackBase` marks where the callee's results will be written. This did not regress relative to the old `callArgs`-based design.

63. **`callArgs` intermediate copy eliminated for local-function path**: The new design avoids `Array(valueStack.suffix(argCount))` in the local-function hot path by reading `valueStack[argsStart...]` directly into `locals`. One unavoidable copy remains for the host-function boundary (`callArgs = Array(valueStack[argsStart...])`). This is correct and intentional per the design comments.
